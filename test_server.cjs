...
app.post('/api/moods', async (req, res) => {
    const { mood, userId, date } = req.body;
    ...
    try {
        ...
        console.error('Mood logging error:', err.message);
...
app.get('/api/moods/user/:userId', async (req, res) => {
    try {
        ...
        const moods = await Mood.find({ userId: userId }).sort({ date: 1 });
...
app.get('/api/journal/user/:userId', async (req, res) => {
    try {
        ...
        let query = JournalEntry.find({ userId: userId });
...
app.get('/api/users/:userId', async (req, res) => {
    try {
        const userId = req.params.userId;
        const user = await User.findById(userId).select('fullName email profilePictureUrl');
        if (!user) {
